...
const SummaryRow = ({ label, value, gold, large, muted }) => (
  <div className={`flex items-center justify-between gap-3 ${large ? 'py-1' : 'py-0.5'}`}>
    <span className={`text-sm leading-snug ${muted ? 'text-white/35' : 'text-white/65'}`}>{label}</span>
    <span className={`font-bold tabular-nums shrink-0
      ${large ? 'text-xl' : 'text-sm'}
      ${gold ? 'text-gold' : muted ? 'text-white/35' : 'text-white/90'}`}>
      {value}
    </span>
  </div>
);
  ...
  return (
    <div className="sticky top-24 flex flex-col gap-4">
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
        <p className="text-white/30 text-[10px] uppercase tracking-widest font-semibold mb-3">Selected Plan</p>
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-white/40 text-xs mt-0.5 truncate">{selectedPlan?.insurerName}</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 flex flex-col gap-4">
        ...
        {/* Base premium */}
        <div>
          <p className="text-white/30 text-[10px] uppercase tracking-widest font-semibold mb-2">Base Premium</p>
          {basePremium != null ? (
            <SummaryRow label="Plan Premium" value={`₹${fmt(basePremium)}`} />
          ) : (
            <p className="text-white/30 text-xs">Trip details needed</p>
          )}
        </div>
        ...
        <div className="rounded-xl bg-gold/[0.08] border border-gold/25 px-4 py-3.5 mt-1">
          <p className="text-white/40 text-[10px] uppercase tracking-widest font-semibold mb-1">Final Premium</p>
          <p className="text-gold text-3xl font-bold tabular-nums">
            {finalPremium != null ? `₹${fmt(finalPremium)}` : '—'}
          </p>
          <p className="text-white/25 text-[10px] mt-1">Inclusive of all selected add-ons</p>
        </div>
      </div>
      ...
      {tripForm && (
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 flex flex-col gap-3">
          <p className="text-white/30 text-[10px] uppercase tracking-widest font-semibold">Trip Summary</p>
